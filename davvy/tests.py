...
class WebDAVTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.user = User.objects.create_user(self.username, 'test@test', self.username)
        self.auth = 'Basic '+base64.b64encode('{}:{}'.format(self.username, self.username))
    ...
    def test_putandget(self, resource_name='services'):
        ...
        self.assertEqual(''.join(response.streaming_content), open('/etc/services').read())
